...
The irony of building this in the `runtimed` org and then moving it to `nteract` isn't lost. But nteract has the Jupyter ecosystem brand recognition, and `runt` is a good CLI name that doesn't need to change just because the org does.
