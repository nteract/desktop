...
/// Check if a pyproject.toml declares ipykernel as a uv-visible dependency.
///
/// Scans (via line walk) the sections uv consults when resolving deps:
/// - `[project]` `dependencies` (PEP 621)
/// - `[dependency-groups]` (PEP 735)
/// - `[tool.uv]` `extra-dependencies`, `dev-dependencies`, `constraint-dependencies`
///
/// Does NOT match inside `[tool.pixi.*]` — that's handled by
/// `pixi_toml_has_ipykernel`. Names are normalized per PEP 503
/// (lowercase, `_`/`.` → `-`), so `IPyKernel`, `ipy_kernel`, and
/// `ipy.kernel` all match. Markers, extras, and version specifiers
/// on PEP 508 requirements are tolerated — only the leading name
/// token is checked.
///
/// Text-level scan (no TOML parse) to avoid pulling in a TOML dep
/// just for this check. A false negative degrades gracefully (launch
/// proceeds, kernel spawn fails with the generic shape).
pub fn pyproject_toml_has_ipykernel(path: &Path) -> bool {
    let Ok(content) = std::fs::read_to_string(path) else {
        return false;
    };
    pyproject_toml_content_has_ipykernel(&content)
}

/// Testable form of `pyproject_toml_has_ipykernel` (no filesystem).
fn pyproject_toml_content_has_ipykernel(content: &str) -> bool {
    #[derive(PartialEq)]
    enum Section {
        Other,
        ProjectDependencies,
    ...
    // Track whether we're inside a multi-line inline list for
    // project.dependencies (`dependencies = [ ... ]` spanning lines).
    let mut in_project_deps_list = false;
    // Track whether we're inside a multi-line inline list under tool.uv
    // for one of the dep-array keys.
    let mut in_tool_uv_deps_list = false;

    for raw_line in content.lines() {
        let line = strip_trailing_comment(raw_line);
        let trimmed = line.trim();

        // Continuation of a multi-line inline list under [project].dependencies.
        if in_project_deps_list {
            if list_contains_ipykernel_inline(trimmed) {
                return true;
            }
        ...
        // Continuation of a multi-line inline list under [tool.uv] dep arrays.
        if in_tool_uv_deps_list {
            if list_contains_ipykernel_inline(trimmed) {
                return true;
            }
            if trimmed.ends_with(']') {
                in_tool_uv_deps_list = false;
            }
            continue;
        }
        ...
        if trimmed.starts_with('[') && trimmed.ends_with(']') {
            ...
            section = if header == "project" {
                Section::ProjectDependencies
            } else if header == "dependency-groups" {
                Section::DependencyGroups
            } else if header == "tool.uv" {
                Section::ToolUvDepsArray
            } else {
                Section::Other
            };
            continue;
        ...
        match section {
            Section::ProjectDependencies => {
                ...
                if let Some(rest) = trimmed.strip_prefix("dependencies") {
                    ...
                    if let Some(rest) = rest.strip_prefix('=') {
                        let rest = rest.trim_start();
                        if let Some(list_body) = rest.strip_prefix('[') {
                            if list_contains_ipykernel_inline(list_body) {
                                return true;
                            }
                            if !list_body.trim_end().ends_with(']') {
            ...
            Section::DependencyGroups => {
                // Any key = [...] list under [dependency-groups] is a group.
            ...
            Section::ToolUvDepsArray => {
                // Only the dep-array keys matter here.
                let is_dep_key = trimmed.starts_with("dev-dependencies")
                    || trimmed.starts_with("extra-dependencies")
                    || trimmed.starts_with("constraint-dependencies");
                if !is_dep_key {
                    continue;
                }
                if let Some(eq_idx) = trimmed.find('=') {
...
/// Strip a trailing `#` comment (TOML) respecting single-line string delims.
///
/// We don't try to be fully spec-correct — just skip obvious
/// `# comment` tails outside of double-quoted strings.
...
/// Scan an inline TOML list body for `ipykernel`.
///
/// `rest` is the text after `[` on a list-opening line, or a
/// continuation line inside a multi-line list. Splits on commas
/// and applies `line_declares_ipykernel` to each entry.
fn list_contains_ipykernel_inline(rest: &str) -> bool {
    ...
    for entry in cleaned.split(',') {
        ...
        if line_declares_ipykernel(entry) {
            return true;
        }
    }
    false
...
/// Check whether a single PEP 508-style requirement string declares ipykernel.
///
/// Extracts the leading name token (before version spec `<>=!~`, extras
/// `[`, environment markers `;`, or whitespace) and normalizes via
/// PEP 503.
fn line_declares_ipykernel(entry: &str) -> bool {
    let name_end = entry
        .find(|c: char| c.is_whitespace() || matches!(c, '=' | '<' | '>' | '!' | '~' | '[' | ';'))
        .unwrap_or(entry.len());
    let name = &entry[..name_end];
    normalize_dist_name(name) == "ipykernel"
}

/// PEP 503 name normalization: lowercase, replace `_` and `.` with `-`.
...
/// Check if an environment.yml declares ipykernel as a conda dep.
///
/// Parses the yaml via `parse_environment_yml`, iterates the returned
/// `dependencies`, and matches the first name token per PEP 503.
/// Pip-subsection entries are already dropped by `parse_environment_yml`
/// (rattler's behavior), so this only detects conda-channel ipykernel.
/// That matches the remediation prompt, which pushes users toward
/// `conda install ipykernel`.
pub fn environment_yml_has_ipykernel(path: &Path) -> bool {
    let config = match parse_environment_yml(path) {
        Ok(c) => c,
        Err(_) => return false,
    };
    for dep in &config.dependencies {
        let name_end = dep
            ...
            .unwrap_or(dep.len());
        if normalize_dist_name(&dep[..name_end]) == "ipykernel" {
            return true;
        }
    }
    false
...
mod tests {
    ...
    // ── pyproject.toml ipykernel detection ────────────────────────────

    #[test]
    fn test_pyproject_toml_has_ipykernel_project_deps_single_line() {
        let content = "[project]\nname = \"demo\"\ndependencies = [\"ipykernel\"]\n";
        assert!(pyproject_toml_content_has_ipykernel(content));
    }

    #[test]
    fn test_pyproject_toml_has_ipykernel_project_deps_multiline() {
        let content = "[project]\nname = \"demo\"\ndependencies = [\n  \"numpy\",\n  \"ipykernel\",\n  \"pandas\",\n]\n";
        assert!(pyproject_toml_content_has_ipykernel(content));
    }

    #[test]
    fn test_pyproject_toml_has_ipykernel_with_version_and_extras() {
        let content = "[project]\nname = \"demo\"\ndependencies = [\"ipykernel>=6\"]\n";
        assert!(pyproject_toml_content_has_ipykernel(content));

        let content2 =
            "[project]\nname = \"demo\"\ndependencies = [\"ipykernel[test]>=6 ; python_version > \\\"3.9\\\"\"]\n";
        assert!(pyproject_toml_content_has_ipykernel(content2));
    }

    #[test]
    fn test_pyproject_toml_has_ipykernel_dependency_groups() {
        let content = "[project]\nname = \"demo\"\n\n[dependency-groups]\ndev = [\"ipykernel\", \"pytest\"]\n";
        assert!(pyproject_toml_content_has_ipykernel(content));
    }

    #[test]
    fn test_pyproject_toml_has_ipykernel_tool_uv_dev() {
        let content =
    ...
    #[test]
    fn test_pyproject_toml_missing_ipykernel() {
        let content = "[project]\nname = \"demo\"\ndependencies = [\"numpy\", \"pandas\"]\n";
        assert!(!pyproject_toml_content_has_ipykernel(content));
    }

    #[test]
    fn test_pyproject_toml_ipykernel_only_in_tool_pixi_not_matched() {
        // ipykernel under [tool.pixi.dependencies] is pixi's concern, not uv's.
        let content = "[project]\nname = \"demo\"\ndependencies = []\n\n[tool.pixi.dependencies]\nipykernel = \"*\"\n";
        assert!(!pyproject_toml_content_has_ipykernel(content));
    }

    #[test]
    fn test_pyproject_toml_has_ipykernel_case_insensitive() {
        // Per PEP 503, name normalization lowercases and collapses `_`/`.`/`-`.
        // `IPyKernel` -> `ipykernel`. `ipy_kernel` / `ipy.kernel` would
        // normalize to `ipy-kernel` — a different distribution — so they
        // must NOT match.
        for variant in ["IPyKernel", "IPYKERNEL", "ipykernel"] {
            let content = format!("[project]\ndependencies = [\"{}\"]\n", variant);
            assert!(
                pyproject_toml_content_has_ipykernel(&content),
                "expected {variant} to match",
            );
        }
        for variant in ["ipy_kernel", "ipy.kernel", "ipy-kernel"] {
            let content = format!("[project]\ndependencies = [\"{}\"]\n", variant);
            assert!(
                !pyproject_toml_content_has_ipykernel(&content),
                "expected {variant} to NOT match ipykernel (different package)",
            );
        }
    }

    #[test]
    fn test_pyproject_toml_has_ipykernel_nonexistent_file() {
        assert!(!pyproject_toml_has_ipykernel(Path::new(
            "/nonexistent/pyproject.toml"
        )));
    }

    // ── environment.yml ipykernel detection ───────────────────────────

    #[test]
    fn test_environment_yml_has_ipykernel_bare_name() {
        let temp = TempDir::new().unwrap();
        write_file(
            temp.path(),
            "environment.yml",
        ...
        assert!(environment_yml_has_ipykernel(
            &temp.path().join("environment.yml")
        ));
    }

    #[test]
    fn test_environment_yml_has_ipykernel_version_pinned() {
        let temp = TempDir::new().unwrap();
        write_file(
            temp.path(),
            "environment.yml",
    ...
    #[test]
    fn test_environment_yml_missing_ipykernel() {
        let temp = TempDir::new().unwrap();
        write_file(
            temp.path(),
            "environment.yml",
    ...
    #[test]
    fn test_environment_yml_has_ipykernel_nonexistent_file() {
        assert!(!environment_yml_has_ipykernel(Path::new(
            "/nonexistent/environment.yml"
        )));
    }
}
