...
impl RuntimeStateDoc {
    ...
    pub fn set_queue(&mut self, executing: Option<&QueueEntry>, queued: &[QueueEntry]) -> bool {
        ...
        let exec_match = match (&cur_exec_cid, &cur_exec_eid, executing) {
            (None, None, None) => true,
            (Some(cid), Some(eid), Some(entry)) => {
                cid == &entry.cell_id && eid == &entry.execution_id
            }
            (Some(cid), None, Some(entry)) => {
                cid == &entry.cell_id && entry.execution_id.is_empty()
            }
            _ => false,
        };
        ...
        let cur_queued_cid_refs: Vec<&str> = cur_queued_cids.iter().map(|s| s.as_str()).collect();
        let cur_queued_eid_refs: Vec<&str> = cur_queued_eids.iter().map(|s| s.as_str()).collect();

        if exec_match && cur_queued_cid_refs == queued_cids && cur_queued_eid_refs == queued_eids {
            return false;
        }
        ...
        for i in (0..cid_len).rev() {
            self.doc
                .delete(&cid_list, i)
                .expect("delete queued cell_id entry");
        }
        let eid_len = self.doc.length(&eid_list);
        for i in (0..eid_len).rev() {
            self.doc
                .delete(&eid_list, i)
                .expect("delete queued execution_id entry");
        }
    ...
    pub fn read_state(&self) -> RuntimeState {
        ...
        let queue_state = queue
            .as_ref()
            .map(|q| {
                let executing_cell_id = self.read_opt_str(q, "executing");
                ...
                QueueState {
                    executing: executing_cell_id.map(|cid| QueueEntry {
                        cell_id: cid,
                        execution_id: executing_exec_id.unwrap_or_default(),
                    }),
                    queued: queued_cell_ids
                        .into_iter()
                        .zip(
                            queued_exec_ids
                                .into_iter()
...
mod tests {
    ...
    fn test_dedup_skips_redundant_writes() {
        ...
        assert!(doc.set_queue(Some(&qe), &q));
        assert!(!doc.set_queue(Some(&qe), &q));
